...
#include "../vector/drns_multiplication.hpp"
...
class DRNSRing {
    ...
    const DRNSSystem<modulus_bits, limbs> multiplier;
    ...
    inline DRNSRing(const BigInt &modulus) : multiplier(make_DRNSSystemMontgomery<modulus_bits, limbs>(modulus, element_modbits, max_bound)) {
        ...
        // fprintf(stderr, "DRNSRing constructor for modulus %zu bits took %zu microseconds\n", modulus.bit_length(), duration.count());
    }
    ...
    template<typename Variant=DRNSVariant<true, false>>
    INLINE
    inline RingElement mul(const RingElement &a, const RingElement &b) const {
        RingElement out;
        multiplier.template mul_reduce<Variant>(a.r_m1, a.r_m2, b.r_m1, b.r_m2, out.r_m1, out.r_m2);
        return out;
    }
    ...
    inline std::array<RingElement, batch> batch_mulreduce(const std::array<RingElement, batch> &a, const std::array<RingElement, batch> &b) const {
        ...
        std::cerr << "Not implemented: batch_mulreduce" << std::endl;
        abort();
    }

    template<int batch, bool serialize_reduce = false> // This is used in batched multiplication
    INLINE
    static inline std::array<RingElement, batch> true_batch_wide_reduce_expand(const std::array<const DRNSRing<modulus_bits, num_additions> *, batch> &these, const std::array<RingElementWide, batch> &a) {
        static_assert(serialize_reduce == false, "serialize_reduce is not supported for DRNSRing");
        std::array<RingElement, batch> result;
        std::array<AVXVector<limbs>, batch> result_m1;
        std::array<AVXVector<limbs>, batch> result_m2;
        std::array<AVXVector<limbs>, batch> a1_lo;
        ...
        std::array<AVXVector<limbs>, batch> a2_lo;
        std::array<AVXVector<limbs>, batch> a2_hi;
        std::array<AVXVector<limbs>, batch> mont_factor_rns;
        std::array<AVXVector<limbs>, batch> p_rns;
        for (int i = 0; i < batch; i++) {
            a1_lo[i] = a[i].lo.r_m1;
            a1_hi[i] = a[i].hi.r_m1;
            a2_lo[i] = a[i].lo.r_m2;
        ...
        for (int i = 0; i < batch; i++) {
            mont_factor_rns[i] = these[i]->multiplier.mont_factor_rns;
            p_rns[i] = these[i]->multiplier.p_rns;
        }
        these[0]->multiplier.intrns2.template reduce_wide_true_batch<batch>(mont_factor_rns, p_rns, a1_lo, a1_hi, a2_lo, a2_hi, result_m2);
        // std::array<const DRNS<MontgomeryReduce, limbs, limbs> *, batch> these_intrns2;
        // for (int i = 0; i < batch; i++) {
        ...
        //     }
        // } else {
        //     DRNS<MontgomeryReduce, limbs, limbs>::template reduce_wide_true_batch<batch>(these_intrns2, a1_lo, a1_hi, a2_lo, a2_hi, result_m2);
        // }
        these[0]->multiplier.intrns2.template batch_expand_wide<batch>(result_m2, result_m1); // Ok since the second CRNS matrix is modulus independent
        for (int i = 0; i < batch; i++) {
            result[i].r_m1 = result_m1[i];
            result[i].r_m2 = result_m2[i];
        }
    ...
    template<int batch, bool serialize_reduce = false>  // This is used in batched multiplication
    INLINE
    static inline std::array<RingElement, batch> true_batch_mulreduce(const std::array<const DRNSRing<modulus_bits, num_additions> *, batch> &these, const std::array<RingElement, batch> &a, const std::array<RingElement, batch> &b) {
        std::array<RingElementWide, batch> result_wide;
        for (int i = 0; i < batch; i++) {
            result_wide[i] = these[0]->wide_mul(a[i], b[i]); // TODO: can we further batch the operations inside?
        }
        return DRNSRing<modulus_bits, num_additions>::template true_batch_wide_reduce_expand<batch, serialize_reduce>(these, result_wide);
    }
