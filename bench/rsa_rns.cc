...
void BM_SpeedRNSRSASign(benchmark::State &state, bssl::Span<const uint8_t> der) {
  RSA_set_use_rns(1);
  ...
  { // Do one warmup round to amortize away the cost of the first signature (which involves computing the CRNS matrix).
...
void BM_SpeedRNSRSAImportKeyAndSign(benchmark::State &state,
...
void BM_SpeedRNSRSAVerify(benchmark::State &state, bssl::Span<const uint8_t> der) {
  RSA_set_use_rns(1);
  bssl::UniquePtr<RSA> key(RSA_private_key_from_bytes(der.data(), der.size()));
  if (!key || RSA_size(key.get()) > kMaxSignature) {
    state.SkipWithError("Failed to parse key.");
  ...
  { // Do one warmup round to amortize away the cost of the first verification (which involves computing the CRNS matrix).
...
void BM_SpeedRNSRSAImportKeyAndVerify(benchmark::State &state,
                                   bssl::Span<const uint8_t> der) {
  RSA_set_use_rns(1);
  bssl::UniquePtr<RSA> key(RSA_private_key_from_bytes(der.data(), der.size()));
  if (!key || RSA_size(key.get()) > kMaxSignature) {
...
void BM_SpeedRNSRSAParsePrivateKey(benchmark::State &state,
                                bssl::Span<const uint8_t> der) {
  RSA_set_use_rns(1);
  for (auto _ : state) {
    bssl::UniquePtr<RSA> key(
...
void BM_SpeedRNSRSAModExpCRT(benchmark::State &state,
                             bssl::Span<const uint8_t> der) {
  RSA_set_use_rns(1);
  bssl::UniquePtr<RSA> key(RSA_private_key_from_bytes(der.data(), der.size()));
  if (!key || RSA_size(key.get()) > kMaxSignature) {
  ...
  // Ensure the key has all CRT parameters set (which should be the case for
  // keys parsed from DER, but verify to ensure we hit the CRT path).
  if (!RSA_get0_p(key.get()) || !RSA_get0_q(key.get()) ||
      !RSA_get0_dmp1(key.get()) || !RSA_get0_dmq1(key.get()) ||
      !RSA_get0_iqmp(key.get())) {
    state.SkipWithError("Key missing CRT parameters.");
    return;
  }

  // Prepare input data: a properly formatted message that's less than the
  // modulus. We'll use a simple padded message.
  const size_t rsa_size = RSA_size(key.get());
  uint8_t input[kMaxSignature];
  uint8_t output[kMaxSignature];

  // Initialize input with a simple pattern (all zeros for simplicity)
  // In practice, this would be a properly padded message, but for benchmarking
  // the CRT operation itself, any valid input works.
  memset(input, 0, rsa_size);
  // Set the first byte to something less than the modulus to ensure it's valid
  input[0] = 0x01;

  // Do one warmup round to ensure Montgomery contexts are initialized
  if (!rsa_default_private_transform(key.get(), output, input, rsa_size)) {
    state.SkipWithError("rsa_default_private_transform failed.");
    return;
  }
...
#define RSA_TEST_SUITE(size, key)                             \
  BENCHMARK_CAPTURE(BM_SpeedRNSRSAParsePrivateKey, size, key)    \
      ->Apply(bssl::bench::SetThreads);                       \
  BENCHMARK_CAPTURE(BM_SpeedRNSRSASign, size, key)               \
      ->Apply(bssl::bench::SetThreads)                       \
      ->Unit(benchmark::kMicrosecond); \
  BENCHMARK_CAPTURE(BM_SpeedRNSRSAImportKeyAndSign, size, key)   \
      ->Apply(bssl::bench::SetThreads);                       \
  BENCHMARK_CAPTURE(BM_SpeedRNSRSAVerify, size, key)             \
      ->Apply(bssl::bench::SetThreads)                       \
      ->Unit(benchmark::kMicrosecond); \
  BENCHMARK_CAPTURE(BM_SpeedRNSRSAImportKeyAndVerify, size, key) \
      ->Apply(bssl::bench::SetThreads);                       \
  BENCHMARK_CAPTURE(BM_SpeedRNSRSAModExpCRT, size, key)          \
      ->Apply(bssl::bench::SetThreads)                       \
      ->Unit(benchmark::kMicrosecond); \
