...
void SetThreads(benchmark::internal::Benchmark *bench) {
  ...
  //   }
  // }
  bench->Threads(1); // No distracting multithreading for now
}
...
#ifndef __OPTIMIZE__
  fprintf(stderr, "WARNING: Running in debug mode. Performance measurements may be inaccurate.\n");
#endif
#ifdef ENABLE_RNS
  fprintf(stderr, "RNS Montgomery code is ENABLED.\n");
  #ifdef USE_AVX512_FALLBACK
  fprintf(stderr, "WARNING: Using AVX512 fallback. Performance measurements for RNS Montgomery code may be inaccurate.\n");
  #endif
#else
  fprintf(stderr, "RNS Montgomery code is DISABLED.\n");
#endif
  if (argc < 1) {
